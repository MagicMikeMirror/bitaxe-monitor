...
function defaultLayout(){return widgetMeta.map(([id])=>({id,collapsed:false,hidden:false}))}function leaveEdit(){let grid=document.querySelector('.grid');grid.classList.remove('layout-edit');[...grid.children].forEach(c=>{c.draggable=false;syncWidgetVisibility(c)});$('layoutEdit').textContent='Anpassen';$('layoutSave').hidden=true}function enterEdit(){let grid=document.querySelector('.grid');grid.classList.add('layout-edit');[...grid.children].filter(c=>c.dataset.widget).forEach(c=>{c.hidden=false;c.draggable=true});$('layoutEdit').textContent='Bearbeitung beenden';$('layoutSave').hidden=false}
