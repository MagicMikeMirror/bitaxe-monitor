...
market_cache = {"updated": None, "btc_eur": None, "block_btc": None, "pool": {}, "network": {}}
...
<div class="card wide"><div class="label">Public Pool</div><div class="value" id="poolHash">—</div><div class="facts"><div class="fact"><span class="sub">Miner</span><b id="poolMiners">—</b></div><div class="fact"><span class="sub">Blöcke gefunden</span><b id="poolBlocks">—</b></div><div class="fact"><span class="sub">Netzwerk</span><b id="networkHash">—</b></div></div></div>
...
async function market(){let r=await fetch('/api/market'),x=await r.json();$('blockBtc').textContent=x.block_btc==null?'— BTC':Number(x.block_btc).toFixed(4)+' BTC';$('btcEur').textContent=eur(x.btc_eur);$('blockEur').textContent=eur(x.block_eur);$('blockHeight').textContent=x.network?.blocks?.toLocaleString('de-DE')||'—';$('poolHash').textContent=rate(x.pool?.totalHashRate);$('poolMiners').textContent=x.pool?.totalMiners?.toLocaleString('de-DE')||'—';$('poolBlocks').textContent=x.pool?.blocksFound??'—';$('networkHash').textContent=rate(x.network?.networkhashps)}
...
def market_poller():
    ...
    while True:
        ...
        try:
            pool = get_json(PUBLIC_POOL_API_URL + "/pool")
            network = get_json(PUBLIC_POOL_API_URL + "/network")
            price = get_json(BTC_PRICE_URL)
            safe_pool = {
                "totalHashRate": pool.get("totalHashRate"),
                "totalMiners": pool.get("totalMiners"),
                "blocksFound": len(pool.get("blocksFound") or []),
                "fee": pool.get("fee"),
            }
            safe_network = {
            ...
            with market_lock:
                market_cache = {"updated": now(), "btc_eur": btc_eur, "block_btc": subsidy,
                                "pool": safe_pool, "network": safe_network}
    ...
    def do_GET(self):
        ...
        if p.path == "/api/market":
            with market_lock:
                ...
                payload["pool"] = dict(market_cache["pool"])
